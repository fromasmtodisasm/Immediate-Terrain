...
CCamera::CCamera(glm::vec3 position, glm::vec3 up, float yaw, float pitch) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), Zoom(ZOOM)
{
...
CCamera::CCamera(float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), Zoom(ZOOM)
{
...
void CCamera::ProcessKeyboard(Movement direction, float deltaTime)
{
  ...
  float velocity = this->MovementSpeed * 0.001;
